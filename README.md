...
Note: when using the `setCustomElementsManifest` of Storybook it is not possible
to omit certain argTypes/Controls, as Storybook generates it's own argTypes and 
merges those with the ones generated by this package. Therefore there is also
the option to use a custom `setCustomElementsManifest`, so that Storybook
doesn't interfer:

```js
import { setCustomElementsManifest } from "wc-storybook-helper";
import customElements from "./path/to/custom-elements.json";
